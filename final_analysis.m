...
gaama = 1.4;
R = 287;
mach_in = linspace(2.75,3.25,100);
...
%% Get properties after inlet!
    ...
    % First step
    [M_12, t_ratio1, p_ratio1,beta1] = shock_relations(mach_in(i),gaama,theta1,0,1,0);
...
%% Get properties from combustor
    phi_in = 0;
    while 1
        [T4,P4,M4,length,fuel_rate,tao] = combustor(T3_prime,P3_prime,M3_prime,phi_in,diffuser_exit_area);
        if M4 >= .32 % This number needs to be changed accordingly. 
            phi(i) = phi_in;
            m_dot_fuel(i) = fuel_rate;
            combustor_length(i) = length;
            combustor_time(i) = tao;
            break
        else
            phi_in = phi_in+.0001;
        end
...
%% Find the overall thurst!!
    [force,one,two,three] = thrust_calcs(P1,P2,P6,T1,T6,mach_in(i),M6,m_dot5,inlet_area,0,exit_area,m_dot_fuel(i));
    thrust(i) = force;
    specific_thrust(i) = one;
    specfic_fuel_consumption(i) = two;
    specfic_impulse(i) = three;

    % Finding the max temperature and pressure
    max_pressure(i) = max([P1,P2,P3,P4,P5,P6,P3_prime]);
    max_temp(i) = max([T1,T2,T3,T4,T5,T6,T3_prime]);
end
